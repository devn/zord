(ns zord.pitch
  (:require [zord.util :refer (chop choose-n reverse-get)]
            [goog.string :as s]
            [clojure.browser.repl])
  ;; (:require-macros [zord.macros :as m])
  )

;; Perfect consonance
;; (m/defratio unison    1/1)
;; (m/defratio octave    2/1)
;; (m/defratio fifth     3/2)

;; Imperfect consonance
;; (m/defratio sixth     5/3)
;; (m/defratio third     5/4)

;; Dissonance
;; (m/defratio fourth    4/3)
;; (m/defratio min-third 6/5)
;; (m/defratio min-sixth 8/5)
...
(defn cents
  ...
  cents."
  [freq n-cents]
  (* freq (.pow js/Math 2.0 (/ n-cents 1200.0))))
...
(defn degree->int
  ...
  (if (some #{degree} (keys DEGREE))
    (degree DEGREE)
    (throw (js/Error. (str "Unable to resolve degree: "
                           degree
                           ". Was expecting a roman numeral in the range :i -> :vii or the nil-note symbol :_")))))
...
(defn degrees->pitches
  ...
  (let [root (note root)]
    (when (nil? root)
      (throw (js/Error. (str "root resolved to a nil value. degrees->pitches requires a non-nil root."))))
    (map (fn [degree]
           (cond
...
(defn hz->midi
  ...
  (.round js/Math (+ 69
                     (* 12
                        (/ (.log js/Math (* freq 0.0022727272727))
                           (.log js/Math 2))))))
...
(defn amp->db
  ...
  [amp]
  (* 20 (.log10 js/Math amp)))

; dbamp
(defn db->amp
  "Convert decibels to linear amplitude."
  [db]
  (.exp js/Math (* (/ db 20) (.log js/Math 10))))
...
(defn nth-equal-tempered-freq
  ...
  [base-freq interval]
  (* base-freq (.pow js/Math 2 (/ interval 12))))
